import os
import threading
from tkinter import BOTH, E, LEFT, RIGHT, W, X, Y, YES, Label
...
def push(cursor, msg, sendButton, frame, stat, mycon, user_id, table_name, s, text=""):
    global STOP, REQUEST_FETCH, REQUEST_PUSH, BUTTON_CLICKED, KEY
    sendButton.bind("<Button-1>", clicked)
    from datetime import datetime
    while not STOP:
        # print(f"{user_id}: ", end='')
        # text = input()
        ...
        if text.strip() != '':
            try:
                ...
                if not mycon.is_connected():
                    ...
                    try:
                        push(cursor, msg, frame, stat, mycon,
        ...
        try:
            ...
            for i in cursor:
                ...
                if i[0] not in MSG:
                    ...
                    # print(
                    #     f"{chat_user}: {' '.join(i[0].split()[0:-2])}\n{user_id}: ", end='')
                    text_ = decrypt(' '.join(i[0].split()[0:-2]), KEY)
